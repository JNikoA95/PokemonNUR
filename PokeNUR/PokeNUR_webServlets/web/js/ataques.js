...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            localStorage.setItem("ataqueslist", JSON.stringify(result));
        }
    });

    var elementos = JSON.parse(localStorage.getItem("ataqueslist"));
    ...
    for (var k in elementos) {
        ...
        html =
                ...
                '<td id="lb_nombre">' + obj.nombre + '</td>' +
                '<td id="lb_tipo_id">' + obj.tipo_id + '</td>' +
                '<td>';
...
$(document).on("click", ".mod", function() {
    ...
    var tuplas = JSON.parse(localStorage.getItem("ataqueslist"));
    for (var k in tuplas) {
        var obj = tuplas[k];
...
$(document).on("click", ".del", function() {
 ...
 function save() {
        ...
        $.post('ataques', {make: obj}, function(responseText) {
            if (responseText != -1) {
                var item = JSON.parse(obj);
                var html =
                        //'<img width="" src="images/' + producto.image + '" alt="' + producto.nombre + '">' +
                        '<td id="lb_nombre">' + item.nombre + '</td>' +
                        '<td id="lb_tipo_id">' + item.tipo_id + '</td>' +
                        '<td>';
                if (permiso.bupdate == true) {
                ...
                if (operacion == 'A') {
                    ...
                    var elementos = JSON.parse(localStorage.getItem("ataqueslist"));
                    elementos.push(item);
                    localStorage.setItem("ataqueslist", JSON.stringify(elementos));                    
                } else {
