...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            localStorage.setItem("itemslist", JSON.stringify(result));
        }
    });

    var elementos = JSON.parse(localStorage.getItem("itemslist"));
    ...
    for (var k in elementos) {
        ...
        html =
                ...
                '<td id="lb_nombreitem">' + obj.nombreitem + '</td>' +
                '<td id="lb_precio">' + obj.precio + '</td>' +
                '<td>';
...
$(document).on("click", ".mod", function() {
    ...
    var tuplas = JSON.parse(localStorage.getItem("itemslist"));
    for (var k in tuplas) {
        var obj = tuplas[k];
        if ($(this).data('id') == obj.codigo_id) {
        $("#codigo_id").val(obj.codigo_id);
        $("#nombreitem").val(obj.nombreitem);
...
$(document).on("click", ".del", function() {
 ...
 function save() {
        ...
        $.post('items', {make: obj}, function(responseText) {
            if (responseText != -1) {
                var item = JSON.parse(obj);
                var html =
                        //'<img width="" src="images/' + producto.image + '" alt="' + producto.nombre + '">' +
                        '<td id="lb_nombreitem">' + item.nombreitem + '</td>' +
                        '<td id="lb_precio">' + item.precio + '</td>' +
                        '<td>';
                if (permiso.bupdate == true) {
                ...
                if (operacion == 'A') {
                    ...
                    var elementos = JSON.parse(localStorage.getItem("itemslist"));
                    elementos.push(item);
                    localStorage.setItem("itemslist", JSON.stringify(elementos));                    
                } else {
