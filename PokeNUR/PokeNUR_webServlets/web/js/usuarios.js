...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            localStorage.setItem("usuarioslist", JSON.stringify(result));
        }
    });

    var elementos = JSON.parse(localStorage.getItem("usuarioslist"));
    ...
    for (var k in elementos) {
        ...
        html =
                ...
                '<td id="lb_username">' + obj.username + '</td>' +
                '<td id="lb_contraseña">' + obj.contraseña + '</td>' +
                '<td>';
...
$(document).on("click", ".mod", function() {
    ...
    var tuplas = JSON.parse(localStorage.getItem("usuarioslist"));
    for (var k in tuplas) {
        var obj = tuplas[k];
        if ($(this).data('id') == obj.codigo_id) {
        $("#codigo_id").val(obj.codigo_id);
        $("#username").val(obj.username);
        $("#contraseña").val(obj.contraseña);
    }
    }
});
...
$(document).on("click", ".del", function() {
 ...
 function save() {
        ...
        $.post('usuarios', {make: obj}, function(responseText) {
            if (responseText != -1) {
                var item = JSON.parse(obj);
                var html =
                        //'<img width="" src="images/' + producto.image + '" alt="' + producto.nombre + '">' +
                        '<td id="lb_username">' + item.username + '</td>' +
                        '<td id="lb_contraseña">' + item.contraseña + '</td>' +
                        '<td>';
                if (permiso.bupdate == true) {
                ...
                if (operacion == 'A') {
                    ...
                    var elementos = JSON.parse(localStorage.getItem("usuarioslist"));
                    elementos.push(item);
                    localStorage.setItem("usuarioslist", JSON.stringify(elementos));                    
                } else {
