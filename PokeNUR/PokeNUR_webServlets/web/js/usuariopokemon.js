...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            localStorage.setItem("usuariopokemonlist", JSON.stringify(result));
        }
    });

    var elementos = JSON.parse(localStorage.getItem("usuariopokemonlist"));
    ...
    for (var k in elementos) {
        ...
        html =
                ...
                '<td id="lb_pokemon_id">' + obj.pokemon_id + '</td>' +
                '<td>';
...
$(document).on("click", ".mod", function() {
    ...
    var tuplas = JSON.parse(localStorage.getItem("usuariopokemonlist"));
    for (var k in tuplas) {
        var obj = tuplas[k];
...
$(document).on("click", ".del", function() {
 ...
 function save() {
        ...
        $.post('usuariopokemon', {make: obj}, function(responseText) {
            if (responseText != -1) {
                var item = JSON.parse(obj);
                var html =
                        //'<img width="" src="images/' + producto.image + '" alt="' + producto.nombre + '">' +
                        '<td id="lb_pokemon_id">' + item.pokemon_id + '</td>' +
                        '<td>';
                if (permiso.bupdate == true) {
                ...
                if (operacion == 'A') {
                    ...
                    var elementos = JSON.parse(localStorage.getItem("usuariopokemonlist"));
                    elementos.push(item);
                    localStorage.setItem("usuariopokemonlist", JSON.stringify(elementos));                    
                } else {
