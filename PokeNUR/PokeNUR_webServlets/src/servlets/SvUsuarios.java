...
public class SvUsuarios extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            usuarios = Usuarios.getRecords(0);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            ...
            if (request.getParameter("make") != null) {
                ...
                usuarios .setaccess_id(usr.getCodigo_id());
                response.setContentType("text/html;charset=UTF-8");
                PrintWriter out = response.getWriter();
                try {
                    out.print(usuarios.insert());
                } catch (SQLException ex) {
                    Logger.getLogger(SvUsuarios.class.getName()).log(Level.SEVERE, null, ex);
                } finally {
                    out.close();
                ...
                try {
                    UserManager usr=(UserManager) request.getSession().getAttribute("SessionData");
                    out.print(new Usuarios(Integer.parseInt(request.getParameter("del").trim()),usr.getCodigo_id()).delete());
                    } catch (SQLException ex) {
                      Logger.getLogger(SvUsuarios.class.getName()).log(Level.SEVERE, null, ex);
                } finally {
                    out.close();
